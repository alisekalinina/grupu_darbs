...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($user_name) && !empty($user_surname) && !is_numeric($user_name) && !empty($P_kods) && !empty($A_mat) && !empty($A_eng) && !empty($A_vid)) {
        ...
        $query = "INSERT INTO users (user_id, user_name, user_surname, p_kods, a_mat, a_eng, a_vid,f_prog, s_prog) VALUES (?, ?, ?, ?, ?, ?, ?,? ,?)";

        // Prepare the SQL statement
        $stmt = mysqli_prepare($con, $query);

        // Bind parameters to the prepared statement
        mysqli_stmt_bind_param($stmt, "sssssssss", $user_id, $user_name, $user_surname, $P_kods, $A_mat, $A_eng, $A_vid, $F_prog, $S_prog);
